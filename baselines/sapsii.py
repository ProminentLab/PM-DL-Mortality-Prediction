...
def getSAPSII(db, hadm_id):
    ...
    for _, icu in icus.iterrows():
        ...
        query = "SELECT c.sapsii_prob FROM sapsii c WHERE c.icustay_id=" + str(icustay_id) + ";"
        rows = db.query(query)
        for _, row in rows.iterrows():
            score = row.sapsii_prob
            break
    return score
...
if __name__== "__main__":
    ...
    result_df.to_csv("../data/results/sapsii_results.csv", index=None)

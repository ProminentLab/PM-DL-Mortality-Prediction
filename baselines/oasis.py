...
def getOASIS(db, hadm_id):
    ...
    for _, icu in icus.iterrows():
        ...
        query = "SELECT c.oasis_prob FROM oasis c WHERE c.icustay_id=" + str(icustay_id) + ";"
        rows = db.query(query)
        for _, row in rows.iterrows():
            score = row.oasis_prob
            break
    return score
...
if __name__== "__main__":
    ...
    result_df.to_csv("../data/results/oasis_results.csv", index=None)

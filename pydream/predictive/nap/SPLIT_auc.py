...
class SPLIT:
    def __init__(self, net, tss_train_file=None, tss_test_file=None, tss_val_file=None, options=None):
        ...
        if tss_train_file is not None and tss_test_file is not None:
            ...
            self.X_val = np.concatenate([self.X_train[:300],self.X_train[2035:]])
            self.X2_val = np.concatenate([self.X2_train[:300], self.X2_train[2035:]])
            self.X3_val = np.concatenate([self.X3_train[:300], self.X3_train[2035:]])
            self.Y_val = np.concatenate([self.Y_train[:300], self.Y_train[2035:]])

            self.X_train = self.X_train[300:2035]
            self.X2_train = self.X2_train[300:2035]
            self.X3_train = self.X3_train[300:2035]
            self.Y_train = self.Y_train[300:2035]
    ...
    class EvaluationCallback(Callback):
        ...
        def on_epoch_end(self, epoch, logs={}):
            ...
            auc = roc_auc_score(self.Y_test, y_prob, average='macro')
            ...
            y_val_prob = self.model.predict([self.X_val, self.X2_val, self.X3_val])
            y_val_pred = y_val_prob.argmax(axis=1)
            auc = roc_auc_score(self.Y_val, y_val_prob, average="macro")
            logs['val_auc'] = auc
